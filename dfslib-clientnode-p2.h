...
public:
    ...
    /**
     * Release write access to the server
     *
     * @param filename
     * @return grpc::StatusCode
     */
    ...
    /**
     * Get the CRC for a file on the server
     *
     * @param filename
     * @return grpc::StatusCode
     */
    grpc::StatusCode compareServerCRC(const std::string &filename);
    ...
    // When receiving a file info from the server, this method will be called
    void syncFiles(std::string filename, uint64_t modified_time, int32_t crc);
};
#endif
